...
{
    let staked_table = borrow_global_mut<StakingStore>(@creator_addr);
...
{
    //let on_chain_config = borrow_global<OnChainConfig>(@creator_addr);
    let staked_table = borrow_global_mut<StakingStore>(@creator_addr);
...
{
    let token_id = token::create_token_id_raw(@collection_creator_addr, utf8(b"changelings"), token_name, 0);
...
{
    let staked_table = borrow_global<StakingStore>(@creator_addr);
